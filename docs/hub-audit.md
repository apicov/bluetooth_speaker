...
> Superseded in part by §16. The AVRCP target, the SPI hop and the
> apply-at-the-output position all still stand; the transport, the taper, the
> default and the output width described below were all replaced on 2026-08-18.
...
## 16. Volume, the second pass (2026-08-18)

§15 got the level to the speakers. This is about it arriving reliably, meaning
something even at the bottom of the slider, and not being full scale when nobody
has said anything. Two symptoms from the floor started it: satellites going
inaudible at low volume while the hub was still audible, and the level jumping up
and then back down across a reconnect.

**The level went to a different set than the audio.** `fan_out()` addresses the
multicast group; `streamer_send_vol()` addressed the client list. A satellite
plays by being in the group, not by being on the list, so anything that cleared
its slot -- `CLIENT_TIMEOUT_US` without a probe, a disassociation, the gap
between re-associating and its first probe -- left it hearing the stream and no
longer hearing the level. Nothing about that is visible: the audio never stops.
The level now goes to the group, so hearing the audio and hearing the level are
the same condition. A group frame is never retried, so the ACK is replaced by
repetition: three copies on a change, one pushed at `client_joined()`, and a 1 Hz
standing repeat on its own `esp_timer` -- not a counter in `fan_out()`, which
would have put a `sendto` in the priority-9 task `dma_starve` exists to watch.

`sync_proto.h` had argued for unicast so the level would not be "held for a DTIM
burst behind the audio it would delay". That was never true on this build: both
roles set `WIFI_PS_NONE`, so nothing is buffered for a DTIM at all.

**Nothing re-stated the level.** `sbc_link_send_vol()` was called from exactly
one place, the AVRCP command carrying a new level, so the hub's copy was a memory
of an event with three silent ways to lose it: a hub reboot, a full bridge ring
(the enqueue has a zero timeout and counts the drop as `s_dropped`), and a CRC
rejection at `sbc_in.c`. The bridge now re-states it on a 5 s heartbeat and on
both connect events. The heartbeat is deliberately dumb -- it covers all three
without detecting any of them, and detecting a hub reboot from that side would
cost more than the byte it protects.

**The default was full scale**, on §15's rule that a missed message should play
loud rather than silent. `tools/soak/logs-soak-20260818-102231/events.csv` has
what that cost written down: `sat1, uptime 5997, VOLUME 20/127` -- a satellite
that played for six seconds before it learnt the level, in a room set to 20 out
of 127, so 32 dB too loud. The rule assumed the two failures were symmetric and
they are not. A briefly silent speaker is one somebody walks over to. A unit now
plays silence until told, with a 30 s fallback to full scale for the case the old
rule was really protecting against -- a hub running a build from before any of
this existed. The fallback is a local playback decision and is never relayed: a
hub broadcasting its own guess would take a floor sitting correctly at −50 dB up
to unity.

**The taper wasted the slider.** Square law, measured against the levels a phone
actually sends (it steps in fives): 5 → −56 dB, 10 → −44, 15 → −37, 20 → −32,
25 → −28, 126 → −0.1. One click near the bottom was 4–7 dB and one near the top
was half a decibel, and the soak captures show this floor being run at 10–25 --
in the worst of it. It is linear in dB now, −60 dB at 1 up to unity at 127,
0.4762 dB a step, from a table generated by `tools/gen_vol_table.py`. The same
loudness moved from about 10 to about 34, and from about 20 to about 60, which is
the point: the working range is now mid-travel.

**And the bottom of the slider destroyed the audio.** The gain was applied to a
16-bit sample and written back into 16 bits, so at −37 dB fewer than ten bits
survived and everything below 71 truncated to 0 or −1 -- signal-correlated
distortion, not noise, which is why a quiet passage at a low setting stopped
sounding like anything. That is the "satellites go mute at low volume" report.
The output is 32-bit now and the multiply is exact at every level; the only thing
attenuation changes is how far above the DAC's own noise floor the signal sits.

Two traps came with the width change and both are guarded rather than remembered.
The splice inserts silence by writing a buffer straight at the DAC, and a length
still computed from `sizeof(int16_t)` would have halved every insert while
sounding exactly right -- an insert is a *timing* correction. The hub's is sized
in the DAC domain behind a `_Static_assert`; the satellite's takes a frame count,
which makes the mistake unavailable rather than merely detected. The two refill
divides are self-falsifying: miss one and the `REFILL` line prints exactly double
its historical frame count, which is the acceptance test on hardware.

A ramp came with all of it, because silent-until-told creates the largest step
this system has -- 0 to a real level, mid-programme. Interpolated per frame
across a chunk, full travel in eight chunks, reset at playback start, which
removes the cold-start edge for free. Not synchronised between units: a 46 ms
fade starting a few ms apart is inaudible, and making them agree would mean
putting the level on the timeline.

Twenty-eight host-test properties now, up from six. Worth naming two. The
widening multiply is checked **exhaustively** -- all 128 levels against all 65536
int16 inputs -- because that is what justifies having no saturation branch: the
claim is not "no overflow was found", it is "there is not one". And the taper is
checked as a *relationship* rather than as digits, by re-deriving the dB step
with floating point in the test that the firmware only ever looks up; a
hand-edited entry fails there. The step bound is split in two on purpose, because
the error is not uniform: at the bottom of a −60 dB curve the entries are
integers a few counts apart (66, 69, 73) and rounding is worth up to 0.09 dB,
while from 16 up it is inside 0.05 dB. One loose bound would have hidden which
half was which.

**A trap worth recording separately.** The test Makefile did not list
`audio_out.h` as a prerequisite, so the first run after the taper changed
re-executed the old binary and passed -- reporting `half slider = q15 8063`,
which is the square law's answer. A stale test that passes is worse than no test.
`audio_out.h` and `sbc_link.h` are prerequisites now.

**Still open.** The §15 suspicion is untouched by all of this: whether a
near-silent stream was being suspended by the phone is still unproven, and the
measurement that would settle it is unchanged.
