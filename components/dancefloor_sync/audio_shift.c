/*
 * The crossfade that hides a catch-up step. One function; see audio_shift.h
 * for why it lives in the component and why the two strands must be the same
 * material a few frames apart.
 *
 * The arithmetic is laid out for shift > 0 (drop) and reached for shift < 0
 * (insert) by the same expressions, because they are the same expressions:
 * in both cases the tail of the output comes from src[i+shift], and the fade
                       ...
                       int shift, unsigned fade, unsigned channels)
{
     ...
     * (linear/linear), so the two weights sum to 1 and a signal crossed with
     * itself at a small offset comes out at roughly its own level -- no dip
     * to hear. Same weight for every channel of a frame; they are one sample
     * of the same moment and must move together. */
